...
class Unclaim extends BaseSubCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            Cache::$factions[$faction]["logs"][time()] = "§n" . $sender->getName() . " §funclaim l'ancien claim §n(ADMIN)";
            Faction::broadcastFactionMessage($faction, "Votre faction vient de perdre son claim (Supprimé par un admin)");

            $sender->sendMessage(Util::PREFIX . "Vous venez d'unclaim le claim de la faction §n" . $faction);
        }
    }
