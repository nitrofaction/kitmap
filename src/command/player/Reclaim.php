    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if (count($inventorys) > 0) {
                ...
                $sender->sendMessage(Util::PREFIX . "Vous ne pourrez avoir vos pack(s) journalier que dans: §n" . $format);
                return;
            }
            ...
            Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "Le joueur §n" . $sender->getName() . " §fvient de recevoir §n" . $pack . " §fpack(s) grace à son reclaim !");
        }
    }
    ...
    private function informationForm(Player $player, string $inventory): void
    {
        ...
        $form = new SimpleForm(function (Player $player, mixed $button) use ($file, $inventory) {
            if ($button === 0) {
                ...
                $player->sendMessage(Util::PREFIX . "Vous venez de récupérer votre inventaire que vous avez perdu le §n" . $data["date"]);
