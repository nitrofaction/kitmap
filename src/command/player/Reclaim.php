...
class Reclaim extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if (count($inventorys) > 0) {
                ...
                $sender->sendMessage(Util::PREFIX . "Vous ne pourrez avoir vos pack(s) journalier que dans: §q" . $format);
                return;
            }
            ...
            $session->addValue(["packs", "Classique"], $pack);
    ...
    private function sendRefundForm(Player $player): void
    {
        ...
        foreach ($inventorys as $key => $value) {
            $form->addButton("Mort par §q" . $value["killer"], -1, "", $key);
        }
    ...
    private function informationForm(Player $player, string $inventory): void
    {
        ...
        $form = new SimpleForm(function (Player $player, mixed $button) use ($file, $inventory) {
            if ($button === 0) {
                ...
                $player->sendMessage(Util::PREFIX . "Vous venez de récupérer votre inventaire que vous avez perdu le §q" . $data["date"]);
