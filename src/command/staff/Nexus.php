...
class Nexus extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        switch ($args["opt"]) {
            case "start":
                if (count($entities) !== 0) {
                    $sender->sendMessage(Util::PREFIX . "Un event nexus est déjà en cours... Vous pouvez l'arrêter avec la commande §n/nexus end");
                    return;
                }

                [$x, $y, $z] = explode(":", Cache::$config["pos"]["nexus"]);
