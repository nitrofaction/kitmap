                ...
                switch ($data[1]) {
                    ...
                    case true:
                        if ($data[0]) {
                            ...
                            $player->sendMessage(Util::PREFIX . "Vous venez de désactiver l'atout §n" . $name);
                        } else {
                            $session->data["atouts"][$name][0] = true;
                            Util::givePlayerPreferences($player);

                            $player->sendMessage(Util::PREFIX . "Vous venez d'activer l'atout §n" . $name);
                        }
                        return;
                }
            });
            ...
            switch ($data[1]) {
                case 0:
                    if ($atout["price"] > $session->data["money"]) {
                        $player->sendMessage(Util::PREFIX . "Vous ne possedez pas assez de pièces pour acheter l'atout §n" . $name);
                        return;
                    }
                ...
                case 1:
                    if ($atout["gem"] > $session->data["gem"]) {
                        $player->sendMessage(Util::PREFIX . "Vous ne possedez pas assez de gemmes pour acheter l'atout §n" . $name);
                        return;
                    }
                    ...
                    $player->sendMessage(Util::PREFIX . "Vous venez d'acheter l'atout §n" . $name . " §favec §n" . $atout["gem"] . " §fgemmes");
                    return;
            }
