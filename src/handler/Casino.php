...
class Casino
{
    ...
    public static function openCasinoForm(): SimpleForm
    {
        $form = new SimpleForm(function (Player $player, mixed $data) {
            if (!is_string($data)) {
                ...
                $player->chat("/coinflip");
                return;
            }

            $formToSend = in_array($data, ["dragon-tower", "mines"]) ? self::openCasinoGameForm($data) : null;
    ...
    public static function openBetForm(Player $player, string $game): CustomForm
    {
        ...
        $form->setTitle(self::getGameName($game));
        $form->addInput(Util::PREFIX . "Choisissez le montant à parier", default: "10000", label: "bet");

        if ($game === "mines") {
            $form->addSlider(Util::PREFIX . "Nombre de mine(s) : ", 2, 24, -1, 3, "mine");
        }
