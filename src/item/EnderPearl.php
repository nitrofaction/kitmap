...
class EnderPearl extends Item
{
    public function onUse(PlayerItemUseEvent $event): bool
    {
        ...
        if ($session->inCooldown("enderpearl")) {
            $player->sendMessage(Util::PREFIX . "Veuillez attendre §n" . ($session->getCooldownData("enderpearl")[0] - time()) . " §fsecondes avant de relancer une nouvelle perle");
            $event->cancel();
            return true;
        } else if ($session->inCooldown("_antipearl")) {
            $player->sendTip(Util::PREFIX . "Veuillez attendre §n" . ($session->getCooldownData("_antipearl")[0] - time()) . " §fsecondes avant de relancer une nouvelle perle");
            $event->cancel();
            return true;
