...
class DominationTask
{
    ...
    public static function getScoreboardLines(): array
    {
        ...
        foreach (Cache::$config["pos"]["domination"] as $key => $value) {
            $i++;
            ...
            $lines[] = "§f" . $key . " §n(" . $status . "§n)";
            $lines[] = "  §r§f§l| §r§n" . $actual . "§" . $i;
        }
        ...
        foreach ($leaderboard as $key => $value) {
            $i++;
            $lines[] = "§f" . $i . ". §n" . Faction::getFactionUpperName($key) . "§f: " . $value;
    ...
    {
        [$minX, $maxX, $minZ, $maxZ, $y] = array_values(Cache::$config["pos"]["domination"][$zone]);
            ...
            switch ($actual[1][0]) {
                case "uncaptured":
                    if (is_null($actual[0])) {
                        ...
                        Main::getInstance()->getServer()->broadcastTip(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName($faction) . " §fest entrain de capturer la zone §n" . $key);
                    } else {
                        ...
                        if (!in_array($actual_faction, $insides)) {
                            ...
                            Main::getInstance()->getServer()->broadcastTip(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName($actual_faction) . " §fa capturé la zone §n" . $key);
                ...
                case "contested":
                    ...
                    if (!in_array($contest_faction, $insides)) {
                        ...
                        Main::getInstance()->getServer()->broadcastTip(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName($contest_faction) . " §fa contesté la zone §n" . $key . " §fappartenant à la faction §n" . Faction::getFactionUpperName($actual[0]));
                ...
                case "captured":
                    ...
                    if (!is_null($contest_faction)) {
                        ...
                        Main::getInstance()->getServer()->broadcastTip(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName($contest_faction) . " §fconteste la zone §n" . $key . " §fappartenant à la faction §n" . Faction::getFactionUpperName($actual[0]));
                    }
                    break;
            }
    ...
    {
        $value = Cache::$config["pos"]["domination"][$zone];
