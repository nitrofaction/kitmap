    ...
    public static function run(): void
    {
        ...
        if (!is_null(Cache::$data["outpost"])) {
            ...
            if (!Faction::exists(Cache::$data["outpost"])) {
                ...
                foreach ($players as $player) {
                    ...
                    if (!$session->data["staff_mod"][0] && $player->isAlive() && Util::insideZone($player->getPosition(), "outpost") && Faction::hasFaction($player) && $session->data["faction"] !== Cache::$data["outpost"]) {
                        ...
                        if ((time() - self::$spam) > 5) {
                            Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName(self::$currentFaction) . " §fest entrain de capturer l'outpost de la §n" . Faction::getFactionUpperName(Cache::$data["outpost"]) . " §f!");
                            self::$spam = time();
                        }
                        return;
                    }
            ...
            if (0 >= self::$currentOutpost) {
                Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName(Cache::$data["outpost"]) . " §fne possède plus l'outpost !");
        ...
        if (is_null(self::$currentFaction)) {
            foreach ($players as $player) {
                ...
                if (!$session->data["staff_mod"][0] && $player->isAlive() && Util::insideZone($player->getPosition(), "outpost") && Faction::hasFaction($player)) {
                    ...
                    if ((time() - self::$spam) > 5) {
                        Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName(self::$currentFaction) . " §fest entrain de capturer l'outpost §f!");
                        self::$spam = time();
                    }
                    return;
                }
        ...
        if (0 >= self::$currentOutpost) {
            Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §n" . Faction::getFactionUpperName(self::$currentFaction) . " §fvient de capturer l'outpost");
